...
def test_resolve_historical_amount_generic_payload_shape_for_investments():
    # _historical_recurring_investment_amount reuses _resolve_historical_amount with a
    # single-field (amount,) payload (recurring investments have no bonus concept). This pins
    # the core regression this reuse exists to prevent: a mid-backfill rate increase must not
    # get applied to months that fall before the change's effective_from.
    history = [
        (10_000, date(2000, 1, 1), 1),  # created-at amount, effective from far in the past
        (30_000, date(2026, 4, 1), 2),  # raised to 30,000 starting 2026-04
    ]
    # A month before the raise (e.g. backfilled Feb contribution) keeps the old 10,000 rate.
    assert _resolve_historical_amount(history, date(2026, 2, 27), (10_000,)) == (10_000,)
    # The month the raise takes effect (and any month after) uses the new 30,000 rate.
    assert _resolve_historical_amount(history, date(2026, 4, 27), (10_000,)) == (30_000,)
...
def test_plan_recurring_expense_postings_no_prior_history_only_applies_current_month_when_day_reached():
    ...
    postings = _plan_recurring_expense_postings(
        ...
        end_year=None,
        end_month=None,
        history=[(1000, 0, None, None, date(2000, 1, 1), 1)],
        fallback_amount=1000,
        fallback_bonus_amount=0,
        fallback_bonus_month_1=None,
        fallback_bonus_month_2=None,
        name="家賃",
        skipped_months=set(),
    )
...
def test_plan_recurring_expense_postings_adds_bonus_amount_on_bonus_months():
    ...
    postings = _plan_recurring_expense_postings(
        ...
        fallback_bonus_month_1=6,
        fallback_bonus_month_2=12,
        name="車のローン",
        skipped_months=set(),
    )
...
def test_plan_recurring_expense_postings_uses_amount_in_effect_on_each_posting_date():
    today = date(2026, 6, 27)
    history = [
        (10_000, 0, None, None, date(2000, 1, 1), 1),
        (20_000, 0, None, None, date(2026, 6, 1), 2),
    ]
    postings = _plan_recurring_expense_postings(
        today=today,
        last_date=date(2026, 4, 27),
...
def test_plan_recurring_expense_postings_uses_bonus_months_in_effect_on_each_posting_date():
    """Regression test for a real bug: bonus months must be resolved per history row (per
    posting date), not taken from the caller's current live bonus_month_1/2. Otherwise,
    changing which months are bonus months would retroactively misapply to months that were
    posted (or are being backfilled) under a different, earlier schedule - e.g. silently
    stripping a correctly-applied June bonus once the schedule later moves to July/January.
    """
    today = date(2026, 8, 27)
    history = [
        (30_000, 100_000, 6, 12, date(2000, 1, 1), 1),  # original schedule: June/December
        (35_000, 100_000, 7, 1, date(2026, 7, 1), 2),  # rent raised + schedule moved to July/January
    ]
    postings = _plan_recurring_expense_postings(
        today=today,
        last_date=date(2026, 5, 27),
        ...
        fallback_amount=30_000,
        fallback_bonus_amount=100_000,
        # The caller's CURRENT live bonus months (July/January) - if these leaked into past
        # months instead of the history row in effect at each date, June would wrongly lose
        # its bonus below.
        fallback_bonus_month_1=7,
        fallback_bonus_month_2=1,
        name="車のローン",
        skipped_months=set(),
    )
    assert postings == [
        (date(2026, 6, 27), 130_000, "車のローン（固定費自動引き落とし・ボーナス加算）"),
        (date(2026, 7, 27), 135_000, "車のローン（固定費自動引き落とし・ボーナス加算）"),
        (date(2026, 8, 27), 35_000, "車のローン（固定費自動引き落とし）"),
    ]
...
def test_plan_recurring_investment_postings_backfills_and_uses_historical_amount():
    today = date(2026, 6, 27)
    history = [
        (10_000, date(2000, 1, 1), 1),
        (30_000, date(2026, 6, 1), 2),
    ]
